...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo { font-family: 'DM Serif Display', serif; font-size: 26px; color: var(--ink); }
  .logo span { color: var(--accent); }

  .nav-tabs { display: flex; gap: 4px; background: var(--paper-dark); padding: 4px; border-radius: 10px; }
  .nav-tab { padding: 8px 18px; border-radius: 7px; border: none; background: transparent; color: var(--ink-soft); font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
  .nav-tab.active { background: var(--white); color: var(--ink); box-shadow: var(--shadow); }

  .card { background: var(--white); border-radius: 16px; padding: 32px; box-shadow: var(--shadow); margin-bottom: 20px; }
  .card-title { font-family: 'DM Serif Display', serif; font-size: 20px; margin-bottom: 24px; color: var(--ink); }

  .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
  .field { display: flex; flex-direction: column; gap: 6px; }
  .field.span2 { grid-column: span 2; }
  label { font-size: 13px; font-weight: 500; color: var(--ink-soft); text-transform: uppercase; letter-spacing: 0.5px; }
  input, textarea, select { padding: 12px 14px; border: 1.5px solid var(--border); border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 15px; color: var(--ink); background: var(--paper); transition: border 0.2s; outline: none; width: 100%; }
  input:focus, textarea:focus, select:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(232,87,42,0.1); background: var(--white); }
  textarea { resize: vertical; min-height: 90px; }

  .btn { padding: 13px 28px; border-radius: 10px; border: none; font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 600; cursor: pointer; transition: all 0.2s; display: inline-flex; align-items: center; gap: 8px; }
  ...
  .setup-box { background: var(--accent-soft); border: 1.5px dashed var(--accent); border-radius: 14px; padding: 28px; margin-bottom: 24px; text-align: center; }
  .setup-box h3 { font-family: 'DM Serif Display', serif; font-size: 18px; margin-bottom: 8px; }
  .setup-box p { font-size: 14px; color: var(--ink-soft); margin-bottom: 16px; line-height: 1.6; }
  .setup-box input { max-width: 420px; margin: 0 auto 12px; display: block; background: var(--white); }

  .line-row { display: grid; grid-template-columns: 2fr 80px 100px auto; gap: 10px; align-items: end; margin-bottom: 10px; }
  .btn-icon { background: transparent; border: none; color: var(--ink-muted); cursor: pointer; padding: 12px; border-radius: 8px; font-size: 18px; transition: all 0.2s; }
  .btn-icon:hover { color: var(--accent); background: var(--accent-soft); }

  .totals { margin-top: 20px; padding-top: 20px; border-top: 1px solid var(--border); }
  .total-row { display: flex; justify-content: space-between; padding: 6px 0; font-size: 15px; color: var(--ink-soft); }
  .total-row.grand { font-size: 18px; font-weight: 600; color: var(--ink); border-top: 1.5px solid var(--border); margin-top: 8px; padding-top: 12px; }

  .ai-output { background: var(--paper); border: 1.5px solid var(--border); border-radius: 12px; padding: 20px; margin-top: 20px; font-size: 14px; line-height: 1.8; white-space: pre-wrap; }
  .ai-label { display: inline-flex; align-items: center; gap: 6px; font-size: 12px; font-weight: 600; color: var(--accent); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 10px; }

  .preview-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 32px; padding-bottom: 24px; border-bottom: 2px solid var(--ink); }
  ...
  .offerte-item { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border: 1.5px solid var(--border); border-radius: 12px; margin-bottom: 10px; background: var(--white); transition: all 0.2s; cursor: pointer; }
  ...
  .avatar { width: 56px; height: 56px; border-radius: 14px; background: var(--accent); display: flex; align-items: center; justify-content: center; font-family: 'DM Serif Display', serif; font-size: 22px; color: white; flex-shrink: 0; }
...
Schrijf een alinea van 3-5 zinnen als begeleidende tekst bij de offerte. Professioneel, warm maar zakelijk. Begin met "Geachte" of "Beste". Eindig met een positieve afsluiting over samenwerking.`;
      ...
      if (!data.content) throw new Error("Onverwachte response: " + JSON.stringify(data));
      ...
      <div className="app">
        ...
        {!apiKey && (
          <div className="setup-box">
            ...
            <button className="btn btn-primary" onClick={() => setApiKey(apiKeyInput)} disabled={!apiKeyInput.startsWith("sk-")}>
              Verbinden
            </button>
          </div>
        )}
          ...
          <>
            ...
            <div className="card">
              ...
              <div className="form-grid">
                ...
                <div className="field"><label>Bedrijf</label><input value={form.klant_bedrijf} onChange={e => setForm(f => ({ ...f, klant_bedrijf: e.target.value }))} placeholder="Optioneel" /></div>
            ...
            <div className="card">
              ...
              <div className="form-grid">
                ...
                <div className="field span2"><label>Omschrijving</label><textarea value={form.project_omschrijving} onChange={e => setForm(f => ({ ...f, project_omschrijving: e.target.value }))} placeholder="Beschrijf het project..." /></div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              {form.regels.map((r, i) => (
                <div className="line-row" key={i}>
                  ...
                  <div className="field">
                    ...
                    <input type="number" value={r.aantal} onChange={e => updateRegel(i, "aantal", e.target.value)} min="0" />
                  </div>
                  <div className="field">
              ...
              <div className="totals">
                ...
                <div className="total-row">
                  <span>BTW&nbsp;
                    <select value={form.btw_pct} onChange={e => setForm(f => ({ ...f, btw_pct: e.target.value }))} style={{ width: "auto", padding: "2px 6px", display: "inline" }}>
            ...
            <div className="card">
              ...
              <div className="form-grid">
                <div className="field">
                  ...
                  <input type="number" value={form.geldigheidsdagen} onChange={e => setForm(f => ({ ...f, geldigheidsdagen: e.target.value }))} />
                </div>
              </div>
        ...
        {tab === "overzicht" && (
          <div className="card">
            ...
            {offertes.length === 0 ? (
              ...
              <div className="offerte-item" key={o.id} onClick={() => { setPreview(o); setTab("preview"); }}>
          ...
          <div className="card" style={{ padding: "40px 48px" }}>
            <div className="preview-header">
              ...
              <div className="preview-meta">
                ...
                Geldig tot: {(() => { const d = new Date(); d.setDate(d.getDate() + parseInt(preview.form.geldigheidsdagen || 30)); return d.toLocaleDateString("nl-NL"); })()}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 8 }}><strong>Betreft: {preview.form.project_titel}</strong></div>
            {preview.form.project_omschrijving && <p style={{ fontSize: 14, color: "var(--ink-soft)", marginBottom: 16 }}>{preview.form.project_omschrijving}</p>}
            ...
            {preview.profile.iban && (
              <p style={{ marginTop: 24, fontSize: 13, color: "var(--ink-muted)" }}>
                Gelieve het bedrag over te maken op <strong>{preview.profile.iban}</strong> onder vermelding van <strong>{preview.nr}</strong>.
              </p>
            )}
